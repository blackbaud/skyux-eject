...
function extractRouteGuardDetails(fileName) {
  const matchRegexp = /@Injectable\s*\([^)]*\)\s*export\s+class\s+([$\w]+)/g;
  const contents = readFile(fileName);

  let result;
  let match;
  while ((match = matchRegexp.exec(contents))) {
    if (result !== undefined) {
      throw new Error(
        `As a best practice, only export one guard per file in "${fileName}".`
      );
    }
  ...
  if (result === undefined) {
    throw new Error(
      `The file "${fileName}" does not export a route guard class, or it is formatted incorrectly.`
    );
  }
...
function assignChildrenToParents(routes) {
  ...
  // Make sure children paths are sorted alphabetically so that
  // "parent" paths are registered before "children" paths.
...
function generateRouteFromIndexFile(indexFile, guards) {
  ...
  const routeParams = routeParamsMatches
    ? routeParamsMatches.map((p) => p.replace(':', ''))
    : undefined;
  ...
  const guard = guards.find((g) => {
    return (
      g.fileName.replace('index.guard.ts', '') ===
      indexComponent.relativeFilePath.replace('index.component.ts', '')
    );
  });
...
function generateRoutesFromIndexFiles(guards) {
  ...
  routes.sort((a, b) => a.path.indexOf(':') - b.path.indexOf(':'));
...
function getRoutesData(skyuxConfig) {
  ...
  // Setup route guard for root route if it exists.
  const rootGuard = guards.find((g) => g.fileName === 'src/app/index.guard.ts');
  if (rootGuard) {
    applyRouteGuardConfig(rootGuard, rootRoute);
  }

  const routesConfig = [rootRoute];

  const routes = generateRoutesFromIndexFiles(guards);
  if (routes.routesConfig.length) {
    rootRoute.children = rootRoute.children.concat(routes.routesConfig);
  }
...
function getRouteComponentImports(routeComponents) {
  return routeComponents
    .map((x) => {
      const importPath = x.relativeFilePath
        .replace('src/app/', './')
        .replace('.ts', '');
      return `import {
  ${x.className}
...
function stringifyRoutesConfig(routes, indentLevel = 1) {
  ...
  routes.forEach((config) => {
    ...
    if (config.redirectTo) {
      str += `, redirectTo: '${config.redirectTo}', pathMatch: '${config.pathMatch}'`;
    }

    if (config.children) {
      const childrenSegments = stringifyRoutesConfig(
        config.children,
        indentLevel + 1
      );
