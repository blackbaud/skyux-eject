...
function copyFile(fileName, destinationRoot) {
  ...
  fs.copySync(resolved, path.join(destinationRoot, resolved.replace(CWD, '')));
}
...
function copyAssetsDirectory(ejectedProjectPath) {
  logger.info('Copying "./src/assets" directory...');
...
/**
 * Copies all styles from the existing SKY UX application to the new Angular CLI application.
 */
function copyStylesDirectory(ejectedProjectPath) {
  logger.info('Copying "./src/styles" directory...');

  const stylesDirectory = path.join(CWD, 'src/styles');
  /*istanbul ignore else*/
  if (fs.existsSync(stylesDirectory)) {
    fs.copySync(stylesDirectory, path.join(ejectedProjectPath, 'src/styles'));
  }
